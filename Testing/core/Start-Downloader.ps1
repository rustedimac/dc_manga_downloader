...
if (Test-Path $ConfigFile) {
    # [FIX] Added double parentheses to force handle closure
    @( (Get-Content -LiteralPath $ConfigFile -Encoding UTF8 -ErrorAction SilentlyContinue) ) | Where-Object { $_ -match '^\s*([^:]+)\s*:\s*(.*)$' } | ForEach-Object {
        $Config[$Matches[1].Trim()] = ($Matches[2] -split '#')[0].Trim(" `"'")
    }
}
$Global:JunkSeriesTitles = @("ETC", "기타", "삭제됨", "단편")
$Global:DeferredExternalLinks = New-Object System.Collections.Generic.List[PSObject] # <--- 이 줄을 추가하세요!
...
function Global:Show-VisualProgress {
    param (
        [int]$Current, 
        [int]$Total,
        [string]$Prefix = "Progress:" # 기본값은 일반 프로그레스 바
    )
    ...
    $Left = $Width - $Done
    $Bar = "[" + ("#" * $Done) + ("-" * $Left) + "]"
    Write-Host -NoNewline "`r    $Prefix $Bar $Percent% ($Current/$Total images)    "
}
...
function Process-Post($RawUrl, $BaseDir) {
    ...
    if ($null -ne $CatalogMatch) {
		...
		$FinalSeriesName = $CatalogName
		Write-Host "  [CATALOG MATCH] Mapped to database: $CatalogName" -ForegroundColor Green
	
		if ($Global:JunkSeriesTitles -contains $FinalSeriesName -or $FinalSeriesName -match "^(?i)(ETC|기타|삭제됨)$") {
        Write-Host "  [SKIP] Junk series strictly filtered: $FinalSeriesName" -ForegroundColor DarkGray
			return $false
		}
    ...
    if ($TotalCount -eq 0) { Write-Host "  ! No images found." -ForegroundColor Yellow }
        ...
        Show-VisualProgress 0 $TotalCount
            ...
            if ($null -eq $ExistingFile) {
                ...
                while (($RunningJobs | Where-Object { $_.State -eq 'Running' }).Count -ge $MaxThreads) { 
                    ...
                    if ($Completed) {
                        ...
                        Remove-Job -Job $Completed -Force -ErrorAction SilentlyContinue 2>$null
                        $RunningJobs = @($RunningJobs | Where-Object { $_.State -eq 'Running' })
                    } else { Start-Sleep -Milliseconds 20 } # [FIX] 50ms -> 20ms로 딜레이 감소
                }
	...
	# ==========================================
    # [FIX] EXTERNAL LINK PIPELINE (변수 선언 위치 복구 및 통합)
    # ==========================================
    $UrlPattern = '(?i)((?:https?://|https?:\\/\\/)?(?:' + $Global:ExtWhitelistPattern + ')[a-zA-Z0-9\-\./\\\?=&_%]+)'
    $AllExternalLinks = @()

    # --- 4. Extra Links in Body ---
    $FinalExtraLinks = ""
    if ($Html -match $Global:ExtWhitelistPattern) {
        $BodyExtLinks = @()
        [regex]::Matches($Html, $UrlPattern) | ForEach-Object {
            $BodyExtLinks += $_.Groups[1].Value
        }
        if ($BodyExtLinks.Count -gt 0) {
            $BodyExtLinks = $BodyExtLinks | Select-Object -Unique
            $FinalExtraLinks = $BodyExtLinks -join "|"
        }
    }
...
# 3. 외부 모듈 호출
    if ($AllExternalLinks.Count -gt 0) {
        ...
        if ($Config.AutoDownloadExternalLinks -ne 'false') {
            ...
            if (Test-Path $ExtScript) {
                ...
                if ($null -ne $ExtResult) {
                    ...
                    if ($null -ne $ExtResult.ResolvedLinks -and $ExtResult.ResolvedLinks.Count -gt 0) {
                        $UniqueResolved = $ExtResult.ResolvedLinks | Select-Object -Unique
                        
                        for ($x=0; $x -lt $script:SessionExtraLinks.Count; $x++) {
                            foreach ($tel in $AllExternalLinks) {
                                if ($script:SessionExtraLinks[$x].Links -match [regex]::Escape($tel)) {
                                    $script:SessionExtraLinks[$x].Links = ($script:SessionExtraLinks[$x].Links -replace [regex]::Escape($tel), "") -replace '\|\s*\|', '|' -replace '^\s*\|\s*', '' -replace '\s*\|\s*$', ''
                                }
                            }
                        }
                        $script:SessionExtraLinks = @($script:SessionExtraLinks | Where-Object { -not [string]::IsNullOrWhiteSpace($_.Links) })

                        foreach ($rLink in $UniqueResolved) {
                            $script:SessionExtraLinks += [PSCustomObject]@{ Manga = $OperatorManga; Chapter = $FolderChapter; Links = $rLink }
                        }
                    } else {
                        # 모듈이 아무것도 해독하지 못했을 경우에만 최후의 수단으로 원본 기록
                        foreach ($rLink in $AllExternalLinks) {
                            $script:SessionExtraLinks += [PSCustomObject]@{ Manga = $OperatorManga; Chapter = $FolderChapter; Links = $rLink }
                        }
                    }
                }
            ...
            Write-Host "  [SKIP] External downloading is disabled in config. Links will be logged only." -ForegroundColor DarkGray
    ...
    if ($PostFail -eq 0 -and ($PostSuccess -gt 0 -or $PostSkip -gt 0)) {
        Invoke-WithFileLock "DownloadList" {
            if (Test-Path $ListFile) {
                $Enc = if ($PSVersionTable.PSVersion.Major -ge 6) { "utf8BOM" } else { "UTF8" }
                $Lines = @( (Get-Content -LiteralPath $ListFile -Encoding UTF8 -ErrorAction SilentlyContinue) )
                $UpdatedLines = @($Lines | Where-Object { $_.Trim() -ne $RawUrl -and $_.Trim() -ne $CleanUrl -and $_.Trim() -ne "#RETRY $CleanUrl" -and $_.Trim() -ne "#DELETED $CleanUrl" })                
                Write-FileAtomic -Path $ListFile -Content $UpdatedLines -Encoding $Enc
            }
        }
...
if ($Mode -eq "AUTO") {
    if (Test-Path $ListFile) {
        ...
        if (Test-Path $ListFile) {
            $Lines = Get-Content $ListFile -Encoding UTF8 -ReadCount 0
            if ($null -ne $Lines) {
...
try {
    ...
    if ($Global:DeferredExternalLinks -and $Global:DeferredExternalLinks.Count -gt 0) {
        Write-Host "`n===========================================================" -ForegroundColor Magenta
        Write-Host "  [!] DEFERRED EXTERNAL LINKS FOUND: $($Global:DeferredExternalLinks.Count) link(s)" -ForegroundColor Yellow
        Write-Host "  Cloudflare 보호로 인해 차단된 외부 링크(Arca/Kone)가 있습니다." -ForegroundColor Yellow
        Write-Host "===========================================================" -ForegroundColor Magenta
        
        $ans = Read-Host "지금 다운로드를 재시도하시겠습니까? (필요시 브라우저가 열립니다) (y/n)"
        if ($ans -match '^y') {
            $ExtScript = Join-Path $PSScriptRoot "Invoke-ExternalDownloader.ps1"
            if (Test-Path $ExtScript) {
                & $ExtScript -ProcessDeferred -MaxThreads $MaxThreads -Headers $Headers -UseProxy $Config.UseProxy -RenameSequential $RenameSequential
            } else {
                Write-Host "  [ERROR] Invoke-ExternalDownloader.ps1 모듈을 찾을 수 없습니다." -ForegroundColor Red
            }
        }
    }
