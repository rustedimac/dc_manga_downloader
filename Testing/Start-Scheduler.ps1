...
# If the user typed new numbers, save them back to config.yaml permanently
if ($ConfigUpdated) {
    $Enc = if ($PSVersionTable.PSVersion.Major -ge 6) { "utf8BOM" } else { "UTF8" }
    $Lines | Set-Content $ConfigFile -Encoding $Enc
    Write-Host "[!] Updated config.yaml with your new intervals.`n" -ForegroundColor Green
}
...
Write-Host "Board Crawler runs every: $BoardCrawlerIntervalHours hour(s)"
...
while ($true) {
    ...
    if ($Now -ge $NextBoard) {
        Write-Host "[$Now] Triggering Board Series Crawler..." -ForegroundColor Green
        & (Join-Path $PSScriptRoot "Search-Crawler.ps1") -RunBoardCrawler
        $NextBoard = $Now.AddSeconds($BoardIntervalSec)
        Write-Host "Next Board Crawler run at: $NextBoard`n" -ForegroundColor DarkGray
    }

    # Sleep for 1 minute before checking the clock again
