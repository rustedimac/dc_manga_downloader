...
for /f "tokens=2 delims=: " %%A in ('findstr /i "ForceLegacyMode" "%CONFIG_FILE%"') do ( set "FORCE_LEGACY=%%A" )
if /i "!FORCE_LEGACY!"=="True" ( set "PS_EXE=powershell.exe" ) else ( where pwsh >nul 2>nul && set "PS_EXE=pwsh.exe" || set "PS_EXE=powershell.exe" )
